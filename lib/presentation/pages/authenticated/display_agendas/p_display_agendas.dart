...
class DisplayAgendasPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) =>
          getIt<DisplayAgendasBloc>()..add(DisplayAgendasMountedEvent()),
      child: BlocListener<DisplayAgendasBloc, DisplayAgendasState>(
        listener: (context, state) {
          if (state.status == Status.error && state.isMounted) {
            ...
            context.read<DisplayAgendasBloc>().add(
              UpdateDisplayAgendasStatusEvent(
